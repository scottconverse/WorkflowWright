...
<main id="top">
  ...
  <div class="strip">
    <div class="wrap strip-in">
      ...
      <span class="fact"><b>96 tests</b>, no network or token spend</span>
...
<span class="t-warn">refusing to scaffold a spec with structural problems:</span>
  ...
  <section id="driver" class="wrap">
    ...
    <ol class="steps">
      ...
      <li>
        <h3>Prompts travel on stdin wherever stdin is read</h3>
        ...
        silently, so the agent answers a fragment and the run still looks fine. One
        backend, Antigravity, ignores stdin; there the prompt goes on the command line
        and an oversized one is <em>refused</em> rather than quietly cut in half.</p>
      </li>
    </ol>
  </section>
  ...
  <section class="wrap">
    ...
    <div class="grid-2">
      <div class="panel">
        ...
        spawns the system its <code>backend</code> names — Claude, Codex, Antigravity, or
        an OpenAI-compatible endpoint — and the run goes start to finish on its own. What
        you want from a terminal, from CI, or from cron.</p>
      </div>
      <div class="panel">
...
make test   <span class="t-dim"># 96 tests, no network</span></pre>
  ...
  <section class="wrap">
    ...
    <div class="prose">
      <p>New and openly developed, with a real test suite: 170 stdlib tests covering the
      ...
      release is <a href="__REPO__/releases/latest">v0.2.1</a>; if you installed the
    ...
    <ul class="limits" style="max-width:64ch">
      ...
      intake step.</li>
      <li><b>Running a workflow <em>unattended</em> needs whichever CLI its nodes name</b>
      on PATH — <code>claude</code>, <code>codex</code>, <code>agy</code>, or a reachable
      OpenAI-compatible endpoint. Designing, rendering, critiquing, and scaffolding never
      call a model at all, and <code>--delegate</code> runs a workflow with no CLI
      involved.</li>
      ...
      <li><b>Anything past those four backends means writing code.</b> A node picks its
      system with a <code>backend</code> field rather than an edit, but reaching a fifth
      one — an SDK, a bespoke service — still means changing <code>run_agent</code> in
      <code>runner.py</code>, the single place the workflow leaves the Python process.</li>
    </ul>
  </section>
